...
from src.source import AudioSource, get_audio_source_collection, resolve_gradio_file
...
class WhisperTranscriber:
    ...
    def perform_extra(self, languageName, urlData, singleFile, whisper_file: str, 
        ...
        whisper_file_path = resolve_gradio_file(whisper_file)

        def custom_transcribe_file(source: AudioSource):
            result = load_transcript(whisper_file_path)
...
def create_ui(app_config: ApplicationConfig):
    ...
    elif app_config.whisper_implementation in ["transformers", "huggingface", "hf"]:
        implementation_name = "Transformers"
    else:
    ...
    ui_description = "Speech recognition for Persian (and 90+ other languages), built on Whisper. "
    ui_description += "The **Persian** models in the dropdown are fine-tuned on the FLEURS Farsi dataset; "
    ui_description += "the remaining entries are the stock multilingual Whisper models. "
    ui_description += "Upload audio or video, paste a URL, or record from the microphone - the transcript "
    ui_description += "can be downloaded as SRT, VTT, TXT or JSON."

    ui_description += "\n\n\n\nFor longer audio files (>10 minutes), keep Silero VAD (Voice Activity Detector) selected in the VAD option."

    ui_description += "\n\n\n\nRunning with the **" + implementation_name + "** backend."

    # Recommend faster-whisper
    if is_whisper:
        ui_description += " For faster inference on GPU, try the `faster-whisper` implementation."
    ...
    demo = gr.TabbedInterface([simple_transcribe, full_transcribe, perform_extra_interface],
                              tab_names=["Simple", "Full", "Extra"], title="Whisper Persian")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--whisper_implementation", type=str, default=default_whisper_implementation, choices=["transformers", "whisper", "faster-whisper"],\
                        help="the Whisper implementation to use")
    parser.add_argument("--compute_type", type=str, default=default_app_config.compute_type, choices=["default", "auto", "int8", "int8_float16", "int16", "float16", "bfloat16", "float32"], \
