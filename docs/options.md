...
The **Persian** entries are fine-tuned models loaded straight from HuggingFace by the `transformers`
backend (the default). "Persian Small" is roughly six times faster than "Persian Large v3", which is
correspondingly more accurate. Add your own by editing `models` in `config.json5`.

The remaining entries are the stock multilingual Whisper models:
...
`--whisper_implementation` selects how the model is executed. `transformers` (the default) runs
HuggingFace checkpoints as-is, `whisper` uses OpenAI's reference implementation and converts
HuggingFace models to a `.pt` file on first use, and `faster-whisper` is the fastest on GPU but only
loads CTranslate2 models. See the README for install instructions per backend.

A few of the options below are specific to one backend, and are noted where that applies.
...
Ignored by the `transformers` backend, which has no equivalent setting.
...
## FP16
Whether to perform inference in fp16. True by default. Only used by the `whisper` backend - the
`transformers` and `faster-whisper` backends use the `--compute_type` command line option instead,
which additionally accepts `float32`, `bfloat16` and `int8`.
