...
class ModelConfig:
    def __init__(self, name: str, url: str, path: str = None, type: str = "whisper",
                 language: str = None, chunk_length_s: float = 0, batch_size: int = 1):
        """
        ...
        language: The language the model was fine-tuned on, shown in the UI as a hint. Optional.
        chunk_length_s: Transformers backend only. 0 uses sequential long-form decoding (most
                        accurate); a positive value enables chunked decoding, which is faster on GPU
                        but can drop or repeat a word at chunk boundaries.
        batch_size: Transformers backend only. Number of chunks decoded in parallel. Only has an
                    effect when chunk_length_s is set.
        """
        self.name = name
        self.url = url
        self.path = path
...
class VadInitialPromptMode(Enum):
    ...
    def from_string(s: str):
        normalized = s.strip().lower() if s is not None else None
