...
def create_whisper_container(whisper_implementation: str, 
    ...
    elif (whisper_implementation in ["transformers", "huggingface", "hf"]):
        from src.whisper.transformersWhisperContainer import TransformersWhisperContainer
        return TransformersWhisperContainer(model_name=model_name, device=device, compute_type=compute_type, download_root=download_root, cache=cache, models=models)
