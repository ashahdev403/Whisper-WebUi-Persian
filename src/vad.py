...
class VadSileroTranscription(AbstractTranscription):
    ...
    def _create_model(self):
        # trust_repo is required from torch 1.12 onwards: without it torch.hub prompts on stdin the
        # first time it fetches the repository, which deadlocks the server, the CLI and Docker.
        # Silero VAD is this project's default VAD, so fetching it is expected rather than a surprise.
